...
This folder contains the processing pipeline and accuracy assessment scripts for the Suam LULC analysis.
## 1. Google Earth Engine (JavaScript)
* **`analysis_gee.js`**: 
    * Handles the extraction of the 82-band feature hypercube.
    * Implements the multi-source consensus logic for training data generation.
    * Executes the five classifiers (RF, GBM, CART, MD, SVM).
    * Performs topographic analysis (Elevation/Slope).

## 2. Python Visualizations (Jupyter Notebook)
* **`Visualizations_SUAM.ipynb`**: 
    * Reads the accuracy metrics from the classification results.
    * Generates high-resolution, publication-ready figures:
        * **Figure 3**: Comparative Confusion Matrices.
        * **Figure 4**: Disagreement Analysis (QD/AD/MAI).
        * **Figure 5**: McNemar's Statistical Significance Test.

## 3. Result Figures (.png)
These files are generated by the Jupyter Notebook and are linked to the main project README for visual reference.
